...
from app.services.zhejiang_service import ZhejiangService
...
zhejiang_bp = Blueprint('zhejiang', __name__, url_prefix='/zhejiang')
...
def index():
    ...
    return render_template('zhejiang.html', default_date=today)


@zhejiang_bp.route('/api/data')
def api_data():
    """API: 获取浙江招标数据"""
    try:
        # 支持单日期或日期范围
        ...
        # 获取数据
        data, total = ZhejiangService.get_data(
        ...
        if date_filter_type == 'range':
            stats = ZhejiangService.get_statistics_range(start_date, end_date)
        else:
            stats = ZhejiangService.get_statistics(query_date)
...
def api_categories():
    """API: 获取分类列表"""
    try:
        categories = ZhejiangService.get_categories()
...
def api_sources():
    """API: 获取来源列表"""
    try:
        sources = ZhejiangService.get_sources()
...
def api_export():
    ...
    try:
        ...
        if highlight_only == 'true':
            ...
            for kw in KeywordService.get_all_keywords():
        ...
        for idx, row in enumerate(rows, 1):
            ...
            has_keyword = any(kw in project_name for kw in KeywordService.get_all_keywords())
        ...
        filename = f"浙江招标数据_{query_date}{filter_suffix}.csv"
