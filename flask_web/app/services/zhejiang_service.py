...
class ZhejiangService:
    ...
    def get_data(date_filter_type='single', date_params=None, category='全部',
        ...
        # 查询数据 - 从 zhejiang_bidding_info 表
        sql = f"""
        SELECT * FROM zhejiang_bidding_info
        WHERE {where_clause}
        ...
        # 查询总数
        cursor.execute(f"SELECT COUNT(*) as total FROM zhejiang_bidding_info WHERE {where_clause}", params)
    ...
    def get_statistics(query_date):
        ...
        if keywords:
            conditions = " OR ".join(["project_name LIKE %s"] * len(keywords))
            cursor.execute(f"""
                SELECT COUNT(*) as highlight FROM zhejiang_bidding_info
                WHERE publish_date = %s AND ({conditions})
            """, [query_date] + [f'%{kw}%' for kw in keywords])
            highlight = cursor.fetchone()['highlight']
        else:
            highlight = 0
            print(f"[ZhejiangService] 警告：没有配置关键词，高亮统计为 0")
