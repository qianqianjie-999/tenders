...
class JiangsuService:
    ...
    def get_data(date_filter_type='single', date_params=None, category='全部',
        ...
        # 查询数据 - 从 jiangsu_bidding_info 表
        sql = f"""
        SELECT * FROM jiangsu_bidding_info
        WHERE {where_clause}
        ...
        # 查询总数
        cursor.execute(f"SELECT COUNT(*) as total FROM jiangsu_bidding_info WHERE {where_clause}", params)
    ...
    def get_statistics(query_date):
        ...
        if not keywords:
            keywords = current_app.config.get('HIGHLIGHT_KEYWORDS', [])
            print(f"[JiangsuService] 使用 fallback 关键词列表，共{len(keywords)}个")

        # 总数
        cursor.execute("SELECT COUNT(*) as total FROM jiangsu_bidding_info WHERE publish_date = %s", (query_date,))
        total = cursor.fetchone()['total']
        ...
        if keywords:
            conditions = " OR ".join(["project_name LIKE %s"] * len(keywords))
            cursor.execute(f"""
                SELECT COUNT(*) as highlight FROM jiangsu_bidding_info
                WHERE publish_date = %s AND ({conditions})
            """, [query_date] + [f'%{kw}%' for kw in keywords])
            highlight = cursor.fetchone()['highlight']
        else:
            highlight = 0
            print(f"[JiangsuService] 警告：没有配置关键词，高亮统计为 0")
